...
**Дашборд — структура (детализация):**
1. Header — приветствие «Привет, {Имя}!» + сегодняшняя дата
2. Hero «План на сегодня» — следующая тренировка: иконка + цвет по типу, название, параметры (время/км/повторы), кнопка «Выполнена»
3. «Выполнение плана на неделю» — галки дней Пн–Вс, счётчик «осталось N», «пройдено X из Y»
4. Быстрые действия — «Создать тренировку», «Открыть календарь»

Граница со статистикой: дашборд — операционный взгляд («что делать сегодня/на этой неделе»), статистика — аналитический («как дела в целом, что менять»). Дублировать метрики «в лоб» нельзя: минуты/км/повторы, тренды, план vs факт — территория статистики, на дашборде их не показываем.

Адаптация: mobile — один столбец контента + футер с иконками (bottom nav: Дашборд/Календарь/Статистика); планшет/десктоп — сетка карточек в 2–3 колонки, навигация — аналог футера (sidebar/top bar, см. раздел 1).
...
*Обновлено: 11.09.2026. В структуру дашборда добавлена детализация блоков, граница со статистикой и адаптация навигации. Следующий шаг на выбор: вёрстка или авторизация (в отдельной ветке `feature/auth`).*